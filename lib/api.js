...
export const authStorage = {
  ...
  getToken: () => {
    ...
    return window.localStorage.getItem("sentiqai_token");
  },
  getUser: () => {
    if (typeof window === "undefined") return null;
...
/** Builds the Authorization header from whatever token is currently stored. */
function authHeader() {
...
export const reviewsApi = {
  ...
  // Writes hit protected routes on the backend, so the JWT goes along for the ride.
  create: (payload) => request("/reviews", { method: "POST", body: payload, headers: authHeader() }),
  update: (id, payload) =>
    request(`/reviews/${id}`, { method: "PUT", body: payload, headers: authHeader() }),
  remove: (id) => request(`/reviews/${id}`, { method: "DELETE", headers: authHeader() }),
  search: (q) => request(`/reviews/search?q=${encodeURIComponent(q)}`),
  stats: () => request("/reviews/stats"),
};

/** AI endpoints (Week 7 — real Gemini-backed sentiment analysis) */
export const aiApi = {
  analyzeSentiment: (payload) => request("/ai/sentiment", { method: "POST", body: payload }),
};
...
export const authApi = {
  ...
  // Not a fetch() call — this is a full-page navigation so the browser
  // can follow Google's redirect chain and come back to /auth/callback.
